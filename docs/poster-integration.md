...
- Poster webhook payload shape/latency vs TRACEPLUGIN events — untested, real gap noted above.
- Poster API rate limits — not published, need empirical testing with a real token.
...
- No chain/franchise multi-branch aggregation equivalent to iiko chain-server — needs its own design pass.
